...
#cardio <- select(cardio, -c("id","cardio"))
#cardio[,c("age", "height", "weight", "ap_hi", "ap_lo")] <- apply(cardio[,c("age", "height", "weight", "ap_hi", "ap_lo")], 2, function(x) as.numeric(as.character(x)))
...
#cardio[,c("gender", "cholesterol", "gluc", "smoke", "alco", "active")]<-factor(cardio[,c("gender", "cholesterol", "gluc", "smoke", "alco", "active")])
summary(cardio)
dim(cardio) # checking the dimensions of the dataset

# no labels at data – cardio
#cardio <- as.matrix(cardio[,]) # data only

# difference between MDS and PCA

# MDS – projection of points to 2d plane while controlling for their similarity
# in other words: MDS attempts to preserve pairwise distances
# MDS does not assume the relationship (+), so can deal with non-linear links
# it is better than PCA for unimodal distributions (like normal distr.) (+)
# in non-metric MDS the distance constraints are relaxed (+)
# but it is iterative optimization (-) and has local optima (-)

# PCA – projection of points to directions of maximum variability
# in other words: PCA finds the directions that have the most variance
# PCA assumes linearity (-), thus may fail in some cases
# but it is non-iterative (+) and has no local optima (+)
# main elements of PCA:
# 1) Eigenvectors: principal axes of maximum variance subspace
# 2) Eigenvalues: variance of projected inputs along principle axes
# 3) Estimated dimensionality: number of significant (nonnegative) eigenvalues
# PCA is generally used as a tool of exploratory data analysis / data visualisation

# one can get elements of PCA with individual functions
...
# PLOTS
#df <- select(cardio, -c(id,cardio))
# 1st plot
#install.packages("GGally")
library(GGally)
ggpairs(cardio)
#ggpairs(as.data.frame(cardio))
# 2nd plot
library(corrplot)
...
# loadings() are understood as the importance of a variable in the factor (composite variable) - they are the correlation between factor and variable. The higher the load, the more important the variable in a given factor, preferably to be min. 0.3-0.5. Loadings are interpreted as weights in composite variables. When correlation of variables is positive, the loadings are positive too. If negative correlation appears, loadings may be negative

xxx<-cardio.s # for easier references 

# prcomp() - PCA basic command from stats::
# allows for centering data around 0 (center=TRUE) by shifting the variables
# allows also for rescaling variance to one (unit) (scale.=TRUE)
# this is in fact internal standardisation

xxx.pca1<-prcomp(xxx, center=FALSE, scale.=FALSE) # stats::
xxx.pca1
xxx.pca1$rotation #only “rotation” part, the matrix of variable loadings

summary(xxx.pca1) # first 2 PCs explain just a small fraction of variance – poor…
...
# displaying the most significant variables that constitute PC – here for PC1
...
# We should look for groups of RC and try to give “umbrella name” for all components
# In this example it is hard – maybe RC2 goes easiest as mainly collection of textile and services (except some meat)
# What important, many products are not important at all in explaining the changes – see rows without numbers
# Anyway, those data are difficult to reduce (see low cumulative variance at RC3) – no wonder, they are selected by Central Statistical Office to monitor inflation rate and they were selected well!
...
plot(xxx.pca4$uniqueness, pch=".", xlim=c(-20, 110), main="Uniqueness of factors – keep it low", sub="Proportion of variance that is not shared with other variables.
...
# “Worst variables” are problematic in analysis, so if not necessary, better remove them
...
# to check values of “rotation matrix” and PCA values
...
# whatever we do in selecting PC (in standard approach), prediction uses all PC…
# in options one can use rank.=2 for 2PC – however, it does not change the result, it only limits the printout
...
# in rotated PCA one can select number of factors for model – and prediction
...
# 2.Having 2D PCA, one can visualise the mutual relations between observations on the plot – one can also cluster the output
